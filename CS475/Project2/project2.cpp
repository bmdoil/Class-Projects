...
main( int argc, char *argv[ ] )
{
         ...
         for( int t = 0; t < NUMSTEPS; t++ )
         {
                 ...
                 grain = "coarse";
                 #pragma omp parallel for default(none), shared(Bodies), schedule(SCHED)
                 ...
                 for( int i = 0; i < NUMBODIES; i++ )
                 {
                          ...
                          Body *bi = &Bodies[i];
                              ...
                              //grain = "fine";
                             // #pragma omp parallel for default(none), shared(i, bi, Bodies), reduction(+:fx, fy, fz), schedule(SCHED)
         ...
         double megaBodies = (NUMBODIES * NUMBODIES * NUMSTEPS) / (time1 - time0) / 1000000;

         // print performance here:::
        
        std::fprintf(results, "%d, %d, %d," ", %s, %lf\n", NUMBODIES, NUMSTEPS, NUMTHREADS, grain, megaBodies);
        std:fclose(results); 
