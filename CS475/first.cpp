...
main( int argc, char *argv[ ] )
{
	...
	/*Doing pairwise multiplication. Doing it on the heap so it won't overflow the stack */
	...
	//									(properties, one device, the device, callback, pass in user data, returned status)
	if( status != CL_SUCCESS )
	...
	//Command queue is where we send things. 
	if( status != CL_SUCCESS )
		fprintf( stderr, "clCreateCommandQueue failed\n" );

	// 5. allocate the device memory buffers:
	//Device is a GPU. The read and write terminology is with respect to the OpenCL device. So CL_MEM_READ_ONLY means that the
	//OpenCL device can only get this data, it can't send it back to the host CPU.
	cl_mem dA = clCreateBuffer( context, CL_MEM_READ_ONLY,  dataSize, NULL, &status );
	//							(the context, how the buffer is restricted, # bytes, buffer data already allocated, returned status)
	if( status != CL_SUCCESS )
	...
	time0 = omp_get_wtime( );
	//ND means could be 1d, 2d or 3d problem
	status = clEnqueueNDRangeKernel( cmdQueue, kernel, 1, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL );
	//								(					# dimensions, global work offset (always NULL), globalWorksize, localWorksize, # events, event wait list, event object)
	if( status != CL_SUCCESS )
	...
	//							(command queue, device buffer, want to black until done?, offset, # byrs, host buffer, #events, event wait list, event object)
	if( status != CL_SUCCESS )
