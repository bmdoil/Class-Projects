...
int dfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
	...
	dequePushBack(stack, source); //Create new stack and push source onto stack
	while (!dequeIsEmpty(stack)) //While stack is not empty
	{
		Vertex* cur = dequeBack(stack); //Set cur to top of stack
		dequePopBack(stack);			// Pop top of stack
		cur->isVisited = 1;				// Set cur to visited
		if (cur == destination)			// If cur == dest, return 1
		{
			dequeDelete(stack);
			return 1;
		}
		for (int i = 0; i < cur->numNeighbors; i++) //For all neighbors of cur, if not visited, push onto stack
		{
...
int bfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
	...
	dequePushBack(queue, source);		//Create new queue and push source onto queue

	while (!dequeIsEmpty(queue))		//While queue is not empty
	{
		Vertex* cur = dequeFront(queue);		//Set cur to front of queue
		dequePopFront(queue);			// Pop top of queue
		cur->isVisited = 1;				// Set cur to visited
		if (cur == destination)			// If cur == dest, return 1
		{
			dequeDelete(queue);
		...
		for (int i = 0; i < cur->numNeighbors; i++) //For all neighbors of cur, if not visited, push onto queue
		{
			if (!(cur->neighbors[i]->isVisited))
			{
